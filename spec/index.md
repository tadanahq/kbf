...
agent can reason with and a human can read: organizations, locations, orders,
the metrics computed over them, and the actions an agent may take when it
notices something.
...
the business has (a customer, a location), a `relation` is a typed
connection between two entities (a customer *places* an order), and a
...
Reading the primitives in the abstract only goes so far. Two packages in
this repository are the worked answer key:

- [`packages/universal-core`](../packages/universal-core): Playbook Zero,
  the base ontology every business shares. Every primitive appears here in
  its root form.
- [`examples/cafe-demo`](../examples/cafe-demo): a fictional single-location
  cafe that extends `universal-core`. It exercises every extension
  mechanic the spec allows: a glossary synonym, a client-configured
  relation, a threshold override, and a net-new vertical metric.

Every YAML example in this spec is copied from one of those two packages
